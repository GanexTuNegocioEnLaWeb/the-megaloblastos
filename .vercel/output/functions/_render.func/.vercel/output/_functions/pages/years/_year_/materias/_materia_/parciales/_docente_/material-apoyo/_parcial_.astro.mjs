...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Index;
  const { year, materia, docente, parcial } = Astro2.params;
  const reWrite = {
    1: "Nro 1",
    2: "Nro 2",
    3: "Nro 3"
  };
  const materialSupports = await db.select().from(SupportMaterials).where(and(eq(SupportMaterials.year, Number(year)), eq(SupportMaterials.materia, materia), eq(SupportMaterials.docente, docente), eq(SupportMaterials.parcial, Number(parcial)))).all();
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": "parciales" }, { "default": ($$result2) => renderTemplate` ${maybeRenderHead()}<section class="w-[80dvw] mx-auto max-w-2xl"> <p data-aos="fade-down" data-aos-delay="50" class="font-bold text-neutral-600 dark:text-neutral-300 text-center leading-none mt-10">Bienvenidos al</p> <h2 data-aos="fade-up" data-aos-delay="50" class="text-2xl sm:text-3xl md:text-4xl uppercase font-black text-neutral-800 dark:text-neutral-50 text-center">material de apoyo</h2> <p data-aos="fade-up" data-aos-delay="100" class="text-center font-bold text-primary mb-5">#Megaloblastos</p> <section data-aos="fade-up" data-aos-delay="150" class="mb-5 flex flex-col justify-center items-center"> <p class="font-bold capitalize text-neutral-800 dark:text-neutral-200">
...
docente: <span class="font-normal capitalize">${docente}</span> </p> <p class="font-bold capitalize text-neutral-800 dark:text-neutral-200">
...
}, "C:/ganex/megaloblastos/src/pages/years/[year]/materias/[materia]/parciales/[docente]/material-apoyo/[parcial]/index.astro", void 0);

const $$file = "C:/ganex/megaloblastos/src/pages/years/[year]/materias/[materia]/parciales/[docente]/material-apoyo/[parcial]/index.astro";
const $$url = "/years/[year]/materias/[materia]/parciales/[docente]/material-apoyo/[parcial]";
